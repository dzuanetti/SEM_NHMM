...
Tempo <- NULL
optim_algo<-"GLMNET"
alfa<-1 # to run lasso or elastic net

setwd("/home/mariella2/Environ") # esse é o elastic net com alpha=0.5

set.seed(10)
seeds <-sample(110000000,R) # Seed number para conseguer os mesmos valores simulados
lambdas <- exp(-seq(from=5.8, to=1, by=-0.2))
# 
  ...
  for (h in 1:length(lambdas)){
    ...
    while ( abs(tolval[val])>tol && val < n_max_iter_EM){
      ...
      for (i in 2:length(S_training)) {
        A_hat_t = Mat_trans(X[i,],BetaArray)
      ...
      for (j in 1:K) {
        for (k in 1:K) {
          if (TransCount[k,j]==0){
            positions = sample(2:length(S_treino), 6)
            for (d in 1:4) {
              S_treino[positions[d]]=j
              S_treino[positions[d]-1]=k
            }
          }
          if (TransCount[k,j]>0 & TransCount[k,j]<6){
            positions = sample(2:length(S_treino), 6)
            for (d in 1:2) {
              S_treino[positions[d]]=j
              S_treino[positions[d]-1]=k
...
## here to introduce de glmnet

      Xtemp1<-as.matrix(rbind(Xtemp11,Xtemp12,Xtemp13)[,-1])
      Xtemp2<-as.matrix(rbind(Xtemp21,Xtemp22,Xtemp23)[,-1])
      Xtemp3<-as.matrix(rbind(Xtemp31,Xtemp32,Xtemp33)[,-1])
#
      resp1<-factor(c(rep("1",nrow(Xtemp11)),rep("2",nrow(Xtemp12)),rep("3",nrow(Xtemp13))))
      resp2<-factor(c(rep("1",nrow(Xtemp21)),rep("2",nrow(Xtemp22)),rep("3",nrow(Xtemp23))))
      resp3<-factor(c(rep("1",nrow(Xtemp31)),rep("2",nrow(Xtemp32)),rep("3",nrow(Xtemp33))))

      fit1 <- coef(glmnet(x = Xtemp1, y = resp1, family = "multinomial", alpha = alfa, lambda=lambda))
      fit2 <- coef(glmnet(x = Xtemp2, y = resp2, family = "multinomial", alpha = alfa, lambda=lambda))
      fit3 <- coef(glmnet(x = Xtemp3, y = resp3, family = "multinomial", alpha = alfa, lambda=lambda))
      ...
      for (i in 1:K){
      	BetaArray[i,,1]=t(as.matrix(fit1[[i]]))
		BetaArray[i,,2]=t(as.matrix(fit2[[i]]))          
		BetaArray[i,,3]=t(as.matrix(fit3[[i]]))}
...
df2<-data.frame(Chain_Length, EQM_Validação_Median_Final, EQM_Validação_Mean_Final, EQM_Validação_SD, EQM_Test_Median_Final, EQM_Test_Mean_Final, EQM_Test_SD)
