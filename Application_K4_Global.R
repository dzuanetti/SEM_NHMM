###########################################################################
#                                                                         #
#            APPLICATION GLOBAL LASSO - 4 HIDDEN STATES                   #
#                                                                         #
###########################################################################
...
#
  ...
  FSM1_B <-function(params){#função a maximizar para achar os Betas_1
    resp <- sum(1 - log(1 + exp(Xtemp11%*%params[1:D])+ exp(Xtemp11%*%params[(D+1):(2*D)]))) + sum((Xtemp12%*%params[1:D]) - log( 1 + exp(Xtemp12%*%params[1:D])+ exp(Xtemp12%*%params[(D+1):(2*D)]) )) + sum((Xtemp13%*%params[(D+1):(2*D)]) - log( 1 + exp(Xtemp13%*%params[1:D])+ exp(Xtemp13%*%params[(D+1):(2*D)]) )) + sum((Xtemp14%*%params[1:D]) - log( 1 + exp(Xtemp14%*%params[1:D])+ exp(Xtemp14%*%params[(D+1):(2*D)]) )) - lambda*(sum(abs(params[2:D])) + sum(abs(params[(D+2):(2*D)])))
  }
  
  FSM2_B <-function(params){#função a maximizar para achar os Betas_2
    resp <- sum(1 - log(1 + exp(Xtemp21%*%params[1:D])+ exp(Xtemp21%*%params[(D+1):(2*D)]))) + sum((Xtemp22%*%params[1:D]) - log( 1 + exp(Xtemp22%*%params[1:D])+ exp(Xtemp22%*%params[(D+1):(2*D)]) )) + sum((Xtemp23%*%params[(D+1):(2*D)]) - log( 1 + exp(Xtemp23%*%params[1:D])+ exp(Xtemp23%*%params[(D+1):(2*D)]) )) + sum((Xtemp24%*%params[1:D]) - log( 1 + exp(Xtemp24%*%params[1:D])+ exp(Xtemp24%*%params[(D+1):(2*D)]) )) - lambda*(sum(abs(params[2:D])) + sum(abs(params[(D+2):(2*D)])))
  }
  
  FSM3_B <-function(params){#função a maximizar para achar os Betas_3
    resp <- sum(1 - log(1 + exp(Xtemp31%*%params[1:D])+ exp(Xtemp31%*%params[(D+1):(2*D)]))) + sum((Xtemp32%*%params[1:D]) - log( 1 + exp(Xtemp32%*%params[1:D])+ exp(Xtemp32%*%params[(D+1):(2*D)]) )) + sum((Xtemp33%*%params[(D+1):(2*D)]) - log( 1 + exp(Xtemp33%*%params[1:D])+ exp(Xtemp33%*%params[(D+1):(2*D)]) )) + sum((Xtemp34%*%params[1:D]) - log( 1 + exp(Xtemp34%*%params[1:D])+ exp(Xtemp34%*%params[(D+1):(2*D)]) )) - lambda*(sum(abs(params[2:D])) + sum(abs(params[(D+2):(2*D)])))
  }

  FSM4_B <-function(params){#função a maximizar para achar os Betas_3
    resp <- sum(1 - log(1 + exp(Xtemp41%*%params[1:D])+ exp(Xtemp41%*%params[(D+1):(2*D)]))) + sum((Xtemp42%*%params[1:D]) - log( 1 + exp(Xtemp42%*%params[1:D])+ exp(Xtemp42%*%params[(D+1):(2*D)]) )) + sum((Xtemp43%*%params[(D+1):(2*D)]) - log( 1 + exp(Xtemp43%*%params[1:D])+ exp(Xtemp43%*%params[(D+1):(2*D)]) )) + sum((Xtemp44%*%params[1:D]) - log( 1 + exp(Xtemp44%*%params[1:D])+ exp(Xtemp44%*%params[(D+1):(2*D)]) )) - lambda*(sum(abs(params[2:D])) + sum(abs(params[(D+2):(2*D)])))
  }
...
n_max_iter_EM_2 = 71
Tempo <- NULL
lag_var = FALSE # specify if we use lagged var or not
