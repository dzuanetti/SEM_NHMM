...
mainDir = paste("/home/gustavo/Documents/Academic/Cenarios/Application/Code/K3/Individual",sep = "")
subDir = paste("C_Lagged_",toString(lag_var),"_Resultados_Application_Individual_K",toString(K),sep = "")
dir.create(file.path(mainDir, subDir), showWarnings = FALSE)
setwd(file.path(mainDir, subDir))

set.seed(4)
lambdas <- seq(0.06, 0.09, by=0.01)
  ...
  for (w in 1:length(lambdas)){
    for (b in 1:length(lambdas)){
      ...
      for (t in 2:length(Y_validation)){
        ...
        #Y_hat_validation[t]<-rnorm(1,mu_hat[S_hat_validation[t]],sigma_hat[S_hat_validation[t]])
      }
      ...
      lasso_lambdas[lasso_iterator, ] <- c(lambdas[h],lambdas[w],lambdas[b])
