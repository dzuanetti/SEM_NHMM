...
mu = c(60,70,80) # vetor com media para as duas Normais
sigma = c(3.5,2.8,4) #Vetor com os desvios padrões para as duas normais
...
for (p in 1:R){
  ...
  lasso_RMSE <- NULL
  lasso_S <- matrix(nrow = length(lambdas)^K, ncol = length(S_validation))
  lasso_Y <- matrix(nrow = length(lambdas)^K, ncol = length(S_validation))
  ...
  # INICIO DO LASSO
  ###############################################
  pb <- txtProgressBar(min = 1, max = length(lambdas)^K, style = 3)
    ...
    for (w in 1:length(lambdas)){
      for (b in 1:length(lambdas)){
        ...
        FSM1 <-function(params){#função a maximizar para achar os Betas_1
          resp <- sum(1 - log(1 + exp(Xtemp11%*%params[1:D])+ exp(Xtemp11%*%params[(D+1):(2*D)]))) + sum((Xtemp12%*%params[1:D]) - log( 1 + exp(Xtemp12%*%params[1:D])+ exp(Xtemp12%*%params[(D+1):(2*D)]) )) + sum((Xtemp13%*%params[(D+1):(2*D)]) - log( 1 + exp(Xtemp13%*%params[1:D])+ exp(Xtemp13%*%params[(D+1):(2*D)]) )) - lambda1*(sum(abs(params[2:D])) + sum(abs(params[(D+2):(2*D)])))
        }
        
        FSM2 <-function(params){#função a maximizar para achar os Betas_2
          resp <- sum(1 - log(1 + exp(Xtemp21%*%params[1:D])+ exp(Xtemp21%*%params[(D+1):(2*D)]))) + sum((Xtemp22%*%params[1:D]) - log( 1 + exp(Xtemp22%*%params[1:D])+ exp(Xtemp22%*%params[(D+1):(2*D)]) )) + sum((Xtemp23%*%params[(D+1):(2*D)]) - log( 1 + exp(Xtemp23%*%params[1:D])+ exp(Xtemp23%*%params[(D+1):(2*D)]) )) - lambda2*(sum(abs(params[2:D])) + sum(abs(params[(D+2):(2*D)])))
        }
        
        FSM3 <-function(params){#função a maximizar para achar os Betas_3
          resp <- sum(1 - log(1 + exp(Xtemp31%*%params[1:D])+ exp(Xtemp31%*%params[(D+1):(2*D)]))) + sum((Xtemp32%*%params[1:D]) - log( 1 + exp(Xtemp32%*%params[1:D])+ exp(Xtemp32%*%params[(D+1):(2*D)]) )) + sum((Xtemp33%*%params[(D+1):(2*D)]) - log( 1 + exp(Xtemp33%*%params[1:D])+ exp(Xtemp33%*%params[(D+1):(2*D)]) )) - lambda3*(sum(abs(params[2:D])) + sum(abs(params[(D+2):(2*D)])))
        }
        ...
        lasso_lambdas[lasso_iterator, ] <- c(h,w,b)
  ...
  Best_Lambdas[p,] <- lasso_lambdas[min_index,]
  ...
  Y_hat_test[1]<-rnorm(1,Mu_Rep[S_hat_test[1]],Sigma_Rep[S_hat_test[1]])# O valor para o primeiro valor observavel
